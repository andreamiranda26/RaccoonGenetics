...
#SBATCH --output=logs/Hypothesis-%j.out          # Output file. %j is replaced with job ID
#SBATCH --error=logs/Hypothesis-%j.err           # Error file. %j is replaced with job ID
...
Rscript -e "rmarkdown::render('Hypothesis-vcf.Rmd', clean=TRUE)"