...
RMD="${ROOT_DIR}/scripts/PopulationGenetics/Structure/Plot_structure_region.Rmd"
...
#SBATCH --time=${TIME}
#SBATCH --ntasks=1
#SBATCH --output=${LOGDIR}/Plot_STRUCTURE_Region-%j.out
#SBATCH --error=${LOGDIR}/Plot_STRUCTURE_Region-%j.err
...
Rscript -e "dir.create('${OUTDIR}', showWarnings=FALSE, recursive=TRUE);
            rmarkdown::render('${RMD}',
                              output_dir='${OUTDIR}',
                              output_file='Structure_ARegion.html')"
EOF

echo "[submitted] RMarkdown render job submitted!"
echo "Outputs will be in: ${OUTDIR}/Structure_ARegion.html"
